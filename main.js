...
function initDraggableDrawer() {
    ...
    if (!drawer || !handle || !appContainer) return;

    let isDragging = false;
    let startY = 0;
    let startHeight = 0;
    const minHeight = 106; 
    const maxHeight = 406; // Roughly half of 812px
    ...
    const onDrag = (e) => {
        ...
        // Control content and backdrop opacity based on height
        ...
        if (content) {
            ...
            content.style.transform = `translateY(${(1 - progress) * 20}px)`;
        }
    ...
    const stopDrag = () => {
        if (!isDragging) return;
        isDragging = false;
        drawer.style.transition = 'height 0.3s cubic-bezier(0.4, 0, 0.2, 1)';
        
        const backdrop = document.getElementById('drawerBackdrop');
        if (backdrop) backdrop.style.transition = 'opacity 0.3s ease';
        ...
        if (currentHeight > minHeight + 100) {
...
/**
 * Collapse drawer to initial state
 */
...
function handleLogin() {
    ...
    const username = usernameInput.value.trim() || '智农用户_888';
    ...
    setTimeout(() => {
        ...
        // Close drawer after login
        const drawer = document.getElementById('bottomDrawer');
        drawer.style.height = '106px';
        drawer.classList.remove('expanded');
        const content = drawer.querySelector('.drawer-content');
        if (content) {
            content.style.opacity = '0';
